...
O género ensina fazendo, e a morte é o professor — barata (WP0: retry < 30 s, nada se perde) precisamente **para poder ser** o professor.
...
| 4:00–5:00 | Clareira com fogueira* e bivaque de orcs ao longe | Respirar, curar, olhar o mundo | Primeira pausa segura; a poção usa-se aqui se sobreviveu arranhado |

*"Fogueira" aqui é só ponto de descanso visual — o sistema de checkpoint/descanso é a pergunta 10, dos donos (`→WP1`/decisão de tom).
...
- **Brutamontes → parry:** armar de ~40 frames (`→WP1` fixa), inconfundível, com som próprio (`→WP12`). Esquivar funciona (nunca se pune a esquiva — é a ferramenta universal), mas o parry mata-o em metade do tempo. **Parry é sempre atalho, nunca chave única** — quem não o dominar passa na mesma, mais devagar. Isto preserva a Lei 1 nos dois sentidos.
...
> `Espaço — esquiva` · `Clique dir. (toque) — aparar` · `E — poção`
...
- A morte no troço de ensino devolve ao início do troço (< 20 s de caminho), nada perdido
- O ecrã de morte é seco e rápido (1,5 s, sem sermão) — a punição é voltar a andar, não esperar
...
| Poção sob pressão | Clareira (calma) → 2.ª zona (pressão) | Toca sala 3 |
| Magia/cargas (Feiticeiro) | Primeiro lanceiro (custo zero de errar) | Toca |
