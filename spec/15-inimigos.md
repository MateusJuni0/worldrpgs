...
> **WP6 · Fable** (31-07-2026). O bestiário completo, construído sobre: as **7 raças** do Rico ([`04-inimigos-chefes.md`](04-inimigos-chefes.md), 31-07 ⏳ falta o Mateus), as restrições que o WP1 impôs a todo o inimigo, e os números dos 3 da fatia já derivados no WP2 ([`11-formulas.md`](11-formulas.md)). Chefes têm documento próprio (WP7); o Vorgar aparece aqui só como referência de zona. Tudo `[FABLE]` salvo indicação.
...
2. **Cada ataque traz a marca `aparável` ou `só esquiva`** (WP1). A língua visual é fixa (WP12): `só esquiva` = brilho vermelho curto; aparável = silhueta sem vermelho.
3. **Velocidade:** patrulha < 5,0 m/s — **fugir a correr é sempre possível e grátis** (WP1). Perseguição ≤ 5,0 m/s sustentada; só o **fecho** (anti-kite dos 4 s, WP1) pode ser mais rápido, e é curto e telegrafado.
4. **Postura 0–100** por inimigo; dano de postura = MV × 10, bash ×2, martelos ×1,5 (WP1/WP5). A zero → **Cambaleio 1,2 s**, ripostável.
5. **Tecto de cena:** ≤ 5 inimigos animados em combate ao mesmo tempo (orçamento do WP12). Encontros desenham-se dentro disto.
6. **Nenhum inimigo é desenhado para ser repetido** (Lei 1: sem grind). O XP é o que é; a razão de voltar a uma zona é o mundo, não a mochila.
