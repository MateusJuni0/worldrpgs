...
> **Estado a 01-08-2026, Tarefa 5:** as perguntas **0, 0b, 1, 2, 3, 4, 8 e 9 fecharam** com a aprovação do Mateus e do Rico. As `[TENSÃO]` 41 e 43–56 continuam dos donos, com proposta e recomendação; o [`74`](74-fecho-da-revisao-2.md) põe baselines M2 ou bloqueios honestos sem as decidir. **50–51 ainda bloqueiam `ready`**, juntamente com o gate de frame integrado.
...
47. ⚠️ **Os cinco acessórios antigos justificavam uma quarta categoria de equipamento?** — `[TENSÃO]` de fantasia, já sem dependência partida. A Tarefa 5 escolheu a saída reversível: quatro sinos foram migrados para anéis existentes e a lanterna para consumível + anel; já não existe `acessorio:*`. **Proposta e recomendação `[CODEX]`:** não reabrir uma quarta categoria só para recuperar nomes; se Mateus + Rico quiserem sinos/lanterna como fantasia própria, primeiro decidem slot, `effect_type`, números, origem e exclusão de mão, e só depois voltam a entrar IDs. A decisão dos donos passa a ser expandir ou não o sistema, não desbloquear os baralhos actuais.
...
### Estado de execução seguro após a Tarefa 5 — não fecha as decisões

| Pergunta | O que existe enquanto se espera | Proposta + recomendação que os donos validam |
|---:|---|---|
| **41** | níveis +1–+5 indisponíveis; runtime só publica nível 0 | manter os três eixos como troca com perda/verbo, nunca bónus líquido; **recomendado** remover redução incondicional de mana |
| **43** | absorção mágica global = 0; `blocked_owner_q43` | afinidade em cada instância, família só define físico/estabilidade; **recomendado** não inventar valores antes da escolha |
| **44/54** | 11 armaduras iniciais activas; 57 futuras `implemented:false`, `effect_type:none` | consolidar rumo a ~30 e só dar efeito com cliente real; **recomendado** não produzir as 57 antes da decisão |
| **45** | compromisso e Eco seguem o baseline do [`74`](74-fecho-da-revisao-2.md) §1.1 | Eco conserva custos não-mana e usa mira actual; **recomendado** validar este comportamento no M2 |
| **46** | dois projécteis e doze formas têm contrato físico completo | começar pelas quatro formas da Fatia 1; **recomendado** congelar números apenas depois do M2 |
| **47/48** | não existe `acessorio:*`; cartas usam anéis/consumível; afinidade é bias não-gating | não reabrir quarta categoria sem verbo próprio; **recomendado** conservar afinidade apenas como recomendação de loot |
| **49** | 12 ameaças têm `unresolved_parameters: []` e baselines `[CODEX]` | uma ameaça de cada família por spike; **recomendado** a zona só passa depois da medição M2 |
| **50** | política antiga continua declarada, mas não aprovada para produção larga | actual + uma transição, estado lógico das restantes; **recomendado** tecto 1,6 GB para mundo/arte |
| **51** | nenhum limite novo foi imposto à fantasia do mago | orçamento global de oito actores; **recomendado** falha legível sem vaga, em vez de exceder Lei 4 |
| **52** | 24 slots: Vorgar implementado; 23 `blocked_owner_q52` sem `enemy_id` | identidades/verbos pelos donos; **recomendado** manter cada slot bloqueado até ficha + espaço + loot + teste |
| **53** | parâmetros completos; máquina LOS/audição/regresso continua M2 | avanço 2 m, chegada 1,2 m, pulso 1 s/cura total e dano readquire; **recomendado** validar a cura total antes de implementar |
| **55** | 70 `effect_type` resolvem em clientes fechados; cinco efeitos incompatíveis foram substituídos | nenhum anel inventa sistema global; **recomendado** abrir travessia/matchmaking só por decisão própria, não por item |
| **56** | só o cajado é prometido: duas mãos, quatro escolas, força/velocidade 1,0 | ficha comum e spike sino/talismã; **recomendado** não voltar a listar os cinco instrumentos antes do spike/slot |
