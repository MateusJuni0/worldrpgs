...
| 🟠 | `[CODEX]` **Corpo feminino e seis silhuetas KayKit já entram por `CharacterVisual.setup(..., body_id, class_id)`, mas falta a integração que pertence à casca:** `game_shell.gd::_update_preview()` ainda não passa o `class_id` do rascunho nem oferece `com kit`/`sem elmo`. Os KayKit são personagens completos num rig Medium de 23 ossos, não as 11 peças modulares sobre os dois corpos UAL; só trazem clips gerais/locomoção. Portanto 12 combinações corpo × classe, encaixe das 11 peças, roll/ataque/magia finais e os 2 conjuntos de voz continuam por provar. **Razão:** não fingir retarget nem editar ficheiros dos outros agentes. **Alternativa descartada:** sobrepor os dois esqueletos e chamar-lhe roupa — duplicaria pele, draw calls e atravessamentos. | conteúdo e integração exigidos pelo [`64`](spec/64-criacao-de-personagem.md) |
