...
**Actualizado: 01-08-2026, Revisão 2.** Mantido pelo **Claude/Codex**. É a lista de tudo o que foi identificado como buraco e **ainda não tem dono**.
...
| 🔴 | ⭐ **Doze feitiços prometem efeitos sem mecanismo executável:** `sinal`, `farol_gelado`, `sutura_clara`, `orvalho_lento`, `pele_de_marmore`, `segundo_folego`, `passo_de_centelha`, `ultimo_veu`, `marca_usurario`, `peso`, `espelho`, `chama_faminta`. As primeiras onze não têm campos de potência/duração/expiração suficientes; `chama_faminta` ainda promete a metade revogada “roubar stamina” sem declarar dano de postura/guarda nem retorno. Implementá-los obriga a inventar números | revisão 1 · [`66`](spec/66-catalogo-de-magia.md) §6 · [`70`](spec/70-fecho-dos-sistemas-de-combate.md) §5 · `spells.json` |
| 🔴 | ⭐ **As melhorias dos 53 feitiços estão sintacticamente completas e semanticamente incompletas:** 53/53 níveis `+1` reduzem mana numericamente; nos níveis `+2`–`+5` há só quatro moldes distintos por nível, incluindo “trocar duração” em fichas sem duração e “incluir parceiro” em efeitos que já o visam. A política depende da pergunta 41 dos donos | revisão 1 · [`40`](spec/40-decisoes-espolio-magia-inventario.md) §10 · [`66`](spec/66-catalogo-de-magia.md) §7 |
| 🔴 | **Escudos foram decididos como diferentes por elemento, mas os dados não o conseguem expressar:** `familias_escudo` só declara físico/estabilidade e `combat.json` aplica 50% mágico global. Falta decidir se afinidade elemental pertence à família ou a cada instância e depois catalogar os valores | revisão 1 · [`37`](spec/37-aneis-e-elementos.md) §2 · `weapons.json`/`combat.json` · pergunta 43 |
| ✅ | ~~**A garantia “fugir funciona sempre” não tinha mecanismo em 18/33 fichas comuns**~~ **RESOLVIDO NA REVISÃO 2** — as 33 fichas cruas declaram `chase_speed` ou o par água/terra e o auto-teste exige todos os valores abaixo dos 5,0 m/s da corrida | revisão 2 · [`01`](spec/01-combate.md) Movimento · [`67`](spec/67-catalogo-do-bestiario.md) §3 · `3a7ab4e` |
| 🔴 | **Cinco garantias visíveis do baralho apontam para acessórios fantasma:** `sino_aviso_kobold`, `sino_ferro_kobold`, `sino_osso_kobold`, `sino_prata_afogado` e `lanterna_violeta_antiga` só existem como cartas obrigatórias `acessorio:*`; não há catálogo, ficha, descrição de item nem cliente que as entregue. O auto-teste resolvia arma/armadura/anel e ignorava o prefixo desconhecido | revisão 1 · [`40`](spec/40-decisoes-espolio-magia-inventario.md) §3 · [`67`](spec/67-catalogo-do-bestiario.md) §5 · `enemies.json` |
| 🔴 | ⭐ **Projécteis e formas de entrega não têm física executável:** 5 ataques `tiro` não declaram velocidade/raio/gravidade/vida, 4 `perseguidor` não declaram velocidade/rotação/raio/vida, e as 12 formas usadas pelos 53 feitiços não fecham colisão, cadência, pulsos ou expiração. `tiro` chega a chamar “instantâneo” ao contacto de um projéctil móvel | revisão 2 · [`36`](spec/36-fisica.md) · [`55`](spec/55-formas-de-feitico.md) · pergunta 46 |
| 🔴 | **Onze guardiões e doze subchefes prometidos pelo mundo não têm ficha:** só `vorgar` resolve. Os slots de guardião futuros estão agora normalizados como placeholders `guardiao_<bioma>_wp7`; os subchefes nem IDs têm. Zona nenhuma depois de Brumal pode fechar o gate de densidade assim | revisão 2 · `world.json` · pergunta 52 |
| 🔴 | **As ameaças ambientais ainda têm 18 parâmetros por decidir.** A Revisão 2 materializou todos os números dedutíveis em `environmental_threat.rules` e enumerou o resto em `unresolved_parameters`; Brumal ainda precisa de rotação, bloqueio de controlo e cooldown, e as outras zonas têm resets/dano/volumes/velocidades equivalentes | revisão 2 · `world.json` · pergunta 49 |
| 🔴 | **As seis habilidades têm verbo estruturado, mas o compromisso comum e o Eco continuam incompletos:** faltam `activation_s`, interrupção, ponto de compromisso e início de cooldown; no Eco faltam alvo e custos não-mana. Implementar sem a pergunta 45 inventa regras e pode abrir lançamento grátis de necromancia | revisão 2 · `abilities.json` · pergunta 45 |
| 🔴 | **Afinidade dos 70 anéis usa nove etiquetas sem namespace nem semântica.** Pode ser recomendação, bias, gating ou efeito; a última leitura viola a Lei 3. Não existe `_affinity_tags` nem consumidor validado | revisão 2 · `equipment.json` · pergunta 48 |
| 🔴 | **A máquina de percepção/retorno dos inimigos não está executável.** A spec dá alguns limites, mas “avança dois passos” não tinha distância e “cura ao chegar” não tem velocidade, quantidade, tempo nem reaquisição; o runtime salta alerta/chamada/LOS/audição/regresso/cura por completo | revisão 2 · [`15`](spec/15-inimigos.md) §2 · pergunta 53 |
| 🔴 | **As 57 armaduras fora da Fatia 1 repetem a mesma não-regra:** “permite escolher uma resposta de `<slot>`”, sem dizer resposta, trigger, efeito, custo ou cliente. Só as 11 iniciais são honestas ao declarar `nenhuma` | revisão 2 · `equipment.json` · perguntas 44 e 54 |
| 🔴 | **Os 70 anéis não têm `effect_type` nem mapa de consumidores; cinco dependem de sistemas proibidos/ausentes.** Agarrar borda, saltar com rajada e corda escalável reabrem travessia; sinal de invocação e manchas de outros jogadores pressupõem matchmaking público. Os números existem, o mecanismo não | revisão 2 · `equipment.json` · pergunta 55 |
| 🔴 | **Cinco dos seis instrumentos mágicos não existem e nem o cajado executa a regra prometida.** Sino, talismã, chama, relicário e híbrido não têm fichas/slots; cajados não declaram `spell_power`/velocidade por forma e o runtime usa dano/tempo fixos do feitiço. “A força e a velocidade vivem no instrumento” não se pode construir a partir dos dados | revisão 2 · [`42`](spec/42-estudo-magia.md) §2 · [`55`](spec/55-formas-de-feitico.md) §3 · pergunta 56 |
| 🔴 | ⚠️ **A prova de animação já falha os gates de estabilidade antes de juntar o jogo:** com 5 actores UAL, p99 = 19,910 ms (**+19%** sobre 16,7) e pior = 21,993 ms (**+32%**); com 10, p99 = 19,861 ms e pior = 22,532 ms. A média de 60,0 fps esconde o jitter. Falta o teste quente integrado 2+5 com IA/VFX/HUD/rede; conteúdo pára se não regressar a verde | revisão 2 · [`23`](spec/23-tecnico.md) §6 · [`24`](spec/24-plano.md) M6 · [`medição`](medicoes/animacao-esqueleto-2026-08-01.json) |
...
| ✅ | ~~**Ligar morte → compra do baralho → recibo/save**~~ **RESOLVIDO NA TAREFA 4** — `Enemy.died` chama compra idempotente; almas, item, índice e recibo são publicados na mesma geração atómica e a falha repõe o snapshot | [`72`](spec/72-materiais-consumiveis-e-economia.md) §4 · 8559 testes correntes |
...
| **45** | Compromisso das habilidades e semântica do Eco | proposta: Eco repete compromisso/tempo e conserva custos não-mana |
| **46** | Física de projécteis e formas de entrega | proposta: schema comum; quatro formas provadas antes das restantes |
| **47–48** | Categoria dos cinco acessórios e afinidade dos anéis | proposta: slots explícitos; afinidade nunca faz gating |
| **49** | Dezoito parâmetros ambientais | proposta: uma família por spike; lista vazia como gate da zona |
| **50–51** | Streaming e orçamento global de actores | proposta: actual + transição; oito actores incluindo invocados |
| **52** | Onze guardiões e doze subchefes sem ficha | proposta: IDs reservados e `implemented:false` até ao conteúdo |
| **53** | Percepção, retorno e cura dos inimigos | proposta: 2 m; regresso pela perseguição; cura total após pulso de 1 s em casa |
| **54–55** | Habilidades de 57 armaduras e clientes dos 70 anéis | proposta: `effect_type` fechado; nenhum item inventa um sistema |
| **56** | Slots, força e velocidade dos instrumentos mágicos | proposta: ficha de equipamento; baseline 1,0 no cajado; spike sino/talismã |
...
| ✅ | ~~⭐ **Uma mão / duas mãos não tinha comando nem estado**~~ **RESOLVIDO NA TAREFA 4** — `T`/`Y`, estado próprio de 12 f interrompível, offhand recolhido e arte seleccionada pela empunhadura | [`70`](spec/70-fecho-dos-sistemas-de-combate.md) §2 · 8559 testes correntes |
...
| ✅ | ~~**Comando / gamepad**~~ — todas as acções nucleares, incluindo câmara, têm teclado/rato + botão/eixo, construídos do mesmo catálogo; conforto/deadzone esperam comando físico no M2 | [`73`](spec/73-fecho-dos-buracos-de-integracao.md) §5 · 8559 testes correntes |
