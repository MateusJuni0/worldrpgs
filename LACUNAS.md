...
| 🟠 | **Ligar o ecrã de subir de nível ao descanso:** `LevelUpScreen.open_for_current()` já escolhe um ponto, mostra dano/PV/DEF/stamina/esquivas/mana/carga antes de confirmar e grava com rollback, mas o dono de `game_shell.gd`/fogueiras tem de o instanciar apenas no ponto de descanso e devolver o foco ao fechar. Não criar uma tecla global nem permitir abrir no mundo: a acção é `interact` no descanso. | `game/src/ui/levelup_screen.gd` · [`20`](spec/20-interface.md) |
...
| 🟠 | **`InventorySystem.load_profile()` falha em saves válidos com slot de arma `null`:** `String(equipment.get("offhand"))` lança erro no Feiticeiro e noutros kits sem mão secundária. `LevelUpModel` normaliza apenas uma cópia para conseguir pré-visualizar Carga sem tocar no save; o dono de `inventory_system.gd` deve aceitar `null` como slot vazio na fronteira comum. | `game/src/autoload/inventory_system.gd:256` · `game/src/ui/levelup_model.gd` |
