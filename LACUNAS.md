...
| 🟠 | **Instrumento livre está decidido mas só o cajado resolve hoje.** `spells.json::_schools.mal.instrument_choice` preserva “dois dedicados ou um híbrido”; `equipment.json::magic_instruments` só contém `cajado` e `weapons.json` não tem relicário/híbrido. Os donos desses catálogos devem criar os dois IDs, com o híbrido a deixar uma mão livre e perder eficácia nas duas funções, sem `allowed_classes`/`class_lock`. | `[DECIDIDO]` Mateus · [`52`](spec/52-mago-do-mal.md) §10 · Lei 3 |
