...
| 🟠 | **`BossVorgar` já é a instância real do marcador final, mas o controlador avançado continua sem contrato compatível.** A subclasse herda o combate funcional de `Enemy`, recebe o ataque real, morre e sai do HUD na cena oficial. Porém ainda procura `vorgar_encounter`, ausente do JSON, e tentaria chamar `ArenaVorgar.setup()`, método que a classe actual não expõe; SEPARAR/JUNTAR continuam por isso desligados e a consola conserva o erro explícito. | `repro-inicio.tscn`, 02-08-2026 · `game/src/enemies/boss_vorgar.gd` · `game/src/world/arena_vorgar.gd` |
...
| 🟠 | **O fio básico de morte do primeiro chefe está provado no jogo, mas uma luta completa de vida cheia continua por medir.** `repro-inicio.tscn` arranca a cena real, equipa outra arma pela UI, envia `attack`, confirma que a instância é `BossVorgar`, vê dano, morte e desaparecimento da barra. Isto prova input → combate → morte → resultado visível sem gravar save; não prova o ritmo de uma tentativa completa nem as mecânicas SEPARAR/JUNTAR bloqueadas acima. | prova integrada **9764/9764** + repro verde, 02-08-2026 |
...
2. **Como se prova:** `attack_family_self_test.gd` passa **177/177**: além das oito famílias/32 recursos e da arbitragem contra `Sword_Attack`, instancia `gameplay.tscn`, usa `move_right` e `dodge_sprint` e observa a barra de vida real. Exige 10/10 falhas laterais sem dano, 10/10 esquivas certas sem dano, 10/10 cedo e 10/10 tarde com dano. A comparação tautológica entre duas expressões temporais iguais foi removida. `weapon_attach_self_test.gd` passa **42/42** sobre a ordem inteira de `test_loadouts`; o agregado continua a exigir a integração alheia indicada abaixo.
...
|---|---|---|
| 🔴 | `BossVorgar` combate, mas não instala SEPARAR/JUNTAR | O dono de `enemies.json`/`ArenaVorgar` deve fornecer `vorgar_encounter` e uma API única compatível; depois repetir a prova por input sem erro de arranque. Não mover sequências ou números de combate para `main.gd`. |
...
| 🟠 | `repro-inicio.tscn` passa, mas a consola não está limpa | Os dois erros de instrumentos e o talismã no `WeaponAttach` de jogo foram fechados. Continuam fora desta tarefa o controlador Vorgar sem `vorgar_encounter`, a conversão de `adaga` em `InventoryMenu._show_detail()` e o preview separado de `weapon_visual.gd`, que ainda avisa não ter modelo de talismã. |
...
|---|---|---|
| 🔴 | ⭐ **A geometria visível já é a única autoridade na árvore `geometria-honesta`, mas a prova ainda não pertence a `VERIFICAR.bat`.** `GameplayCue` constrói um único polígono dos campos `range/arc_degrees`, `radius`, `lunge_distance` e raio de projéctil/corpo, entrega o mesmo polígono ao mesh e a `covers_world_point()`; `Enemy` deixou de somar `body_radius` ou reconstruir cone/raio e, sem cue, não causa dano. A prova dedicada instancia `gameplay.tscn`, observa o HUD e passa alcance declarado + sem cue + lateral/certa/cedo/tarde 10/10. Até o dono ligar o comando acima ao gate, não se declara o achado fechado. | `game/src/enemies/enemy.gd` · `game/src/combat/gameplay_cue.gd` · `game/src/player/attack_family_self_test.gd` · ownership de `game/VERIFICAR.bat` |
