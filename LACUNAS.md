...
| 🔴 | **A garantia “fugir funciona sempre” não tem mecanismo em 18/33 fichas comuns:** só 15 tipos declaram `chase_speed` ou o par água/terra; sem a coluna não se consegue provar que todos perseguem abaixo dos 5,0 m/s da corrida. Falta preencher as 18 velocidades e tornar `<5,0` obrigatório no auto-teste | revisão 1 · [`01`](spec/01-combate.md) Movimento · [`67`](spec/67-catalogo-do-bestiario.md) §3 · `enemies.json` |
...
| ✅ | ~~**Ligar morte → compra do baralho → recibo/save**~~ **RESOLVIDO NA TAREFA 4** — `Enemy.died` chama compra idempotente; almas, item, índice e recibo são publicados na mesma geração atómica e a falha repõe o snapshot | [`72`](spec/72-materiais-consumiveis-e-economia.md) §4 · 8435 testes correntes |
...
| ✅ | ~~⭐ **Uma mão / duas mãos não tinha comando nem estado**~~ **RESOLVIDO NA TAREFA 4** — `T`/`Y`, estado próprio de 12 f interrompível, offhand recolhido e arte seleccionada pela empunhadura | [`70`](spec/70-fecho-dos-sistemas-de-combate.md) §2 · 8435 testes correntes |
...
| 🟠 | ⭐ **A selecção da Fatia 1 está importada e as cápsulas visuais foram substituídas por corpos Quaternius animados; as `CapsuleShape3D` de física ficaram byte-a-byte com as dimensões anteriores.** Entraram 35 ficheiros/22,4 MiB: corpo + animações Quaternius, oito malhas Kenney de Brumal e oito módulos KayKit da Toca. ⚠️ `spec/22` escolhe Quaternius Monsters para os orcs, mas esse pack não existe em `art/models/`; até entrar, lanceiro/brutamontes/Vorgar usam escala e cor provisórias do corpo-base para preservar autor e o esqueleto comum de 65 ossos | fase 1.2, 01-08 · `game/assets/models/ASSETS.md` |
...
| ✅ | ~~**Comando / gamepad**~~ — todas as acções nucleares, incluindo câmara, têm teclado/rato + botão/eixo, construídos do mesmo catálogo; conforto/deadzone esperam comando físico no M2 | [`73`](spec/73-fecho-dos-buracos-de-integracao.md) §5 · 8435 testes correntes |
