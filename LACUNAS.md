...
| 🟠 | `StartingLoadouts` ainda viola a regra de catálogo no próprio módulo | Remover `ACTIVE_ORIGIN_IDS` como autoridade e descobrir as origens activas em `loadouts`, excluindo apenas chaves editoriais `_...`; o ensaio isolado continua **0/1**, apesar de o caminho jogável dos sete kits estar verde. |
| 🟠 | `GameData` rejeita `sino` e `talisma` como armas inexistentes | Unificar a validação de `weapons.loadouts.*.offhand` com `equipment.magic_instruments`; não duplicar IDs nem hardcodar excepções no jogador/HUD. |
