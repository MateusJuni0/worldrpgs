...
| 🟠 | **A casca ainda não instancia o ciclo novo:** cada spawn de `main.gd` precisa de meta `placement_id=zone_id:stable_spawn_id`; HP zero deve chamar `ProgressionRuntime.commit_death()`, criar `SoulStain` a partir de `active_soul_stain()` e usar o checkpoint; a fogueira embutida ainda ignora o tecto de dez | integração fora da árvore do dono · `game/src/main.gd` |
| 🟠 | **O menu da fogueira ainda não chama as operações expostas:** `Bonfire.level_up(attribute_id)` e `Bonfire.kindle_ember(...)` estão prontos, mas a escolha de atributo/confirmar custo e a acção de queimar Brasa pertencem à casca; não há gate de nível | integração fora da árvore do dono · `game/src/ui/game_shell.gd` |
...
| 🟠 | **Abrir o ecrã completo no jogo:** `EquipmentScreen` entrega mãos, nove slots canónicos, anéis e atalhos, lista só objectos compatíveis da mochila, roda o boneco e mostra a carga antes de confirmar. O dono de `inventory_menu.gd` deve fazer `_equipment_action()` abrir este ecrã focado no slot do objecto em vez de equipar às cegas; o dono de `game_shell.gd` deve montá-lo/fechá-lo. Não há tecla nova: `inventory_menu` continua a abrir a mochila | entrega do ecrã completo 01-08 · `game/src/ui/equipment_screen.gd` |
...
| 🟠 | **Activar o kit do Mago do Mal sem inventar armas.** A ficha `evil_mage`, o traço de mana, os feitiços iniciais, a habilidade e a postura executável já existem nos ficheiros da árvore `mago-do-mal`; `weapons.json::loadouts._pending_mago_do_mal` continua sem `main/offhand` porque faltam os IDs de relicário/híbrido no catálogo do dono de armas. Esse dono deve criar as fichas, manter acesso por qualquer origem (Lei 3), trocar o placeholder por `loadouts.evil_mage` e actualizar `StartingLoadouts.ACTIVE_ORIGIN_IDS`. | `DECISOES.md` 01-08 · `game/src/classes/dark_mage.gd` · fronteira de propriedade |
...
| 🔴 | **A casca ainda não chama as 12 formas executáveis.** `player.gd::_release_spell()` continua a despachar apenas os tipos legados `projectile`/`aoe`/`barrier` para `combat/spell.gd`; o dono de `game/src/player/player.gd` tem de trocar essa fronteira por `SpellDeliveryFactory.create(...)`, passando o `vfx_bundle` da residência dos favoritos. Esta árvore não pode editar esse ficheiro nem `spell_wheel.gd` | `game/src/player/player.gd:864` · `game/src/spells/spell_delivery_factory.gd` |
...
| 🟠 | **Integração pertence aos donos de magia/inventário/armas:** `spells.json` continua a declarar só cajado nas quatro escolas; `player.gd::_start_cast()` só aceita `main_weapon.can_cast`; `weapons.json::staff` continua a duas mãos e os loadouts não equipam os seis itens secundários. Devem consumir `InstrumentCatalog.build_cast_plan()`, classificar o papel de cada feitiço (`direct/support/raise/drain`), restaurar os IDs por escola, guardar o instrumento no slot decidido e mudar o cajado runtime para uma mão. Até essa costura, a ficha é executável e testada, mas os seis novos itens ainda não aparecem no loop jogável. Não alterado aqui por fronteira explícita de propriedade. | `game/src/spells/instrument_catalog.gd` · pergunta 56 · decisão de mão do Mateus |
