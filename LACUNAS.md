...
| ✅ | ~~**O menu de rede não tem tecla.**~~ **RESOLVIDO 02-08** — a integração corrente deixa o rato livre à entrada, torna JOGAR A DOIS clicável e conserva F3 para reabrir o menu depois de o rato ser capturado. A prova de dois processos clica nos botões reais, suspende/devolve o input local e confirma também que Entrar vazio escreve “Falta o endereço” no ecrã | `main.gd::_unhandled_input` · `game/src/coop/coop_online_gameplay_proof.tscn` |
...
| 🔴 | ⚠️ **O limitador de 60 de `settings_system.gd` colide com FIFO:** na zona completa limpa, FIFO + `Engine.max_fps=60` deu p99 **29,691 ms** e 1% low 32,9; cap 120 deu p99 **16,666 ms** e 1% low 56,6, mas ainda um pico isolado de 33,33 ms. `[CODEX]` recomenda omissão 120 quando FIFO está activo (razão: única variante que fez passar p99 sem cortar imagem; sem cap foi rejeitado a 19,469 ms). **Falta o dono de `game/src/autoload/settings_system.gd` consumir `presentation.recommended_engine_cap_with_fifo`; esta árvore não pode editar esse ficheiro.** A ferramenta isolada corrigida confirmou p99 **6,717 ms** sem VSync e **16,666 ms** com FIFO+cap120; o pacing isolado passa. | [`PERF`](game/PERF.md) · [`74`](spec/74-fecho-da-revisao-2.md) §5 · [`animation_benchmark.gd`](game/src/tools/animation_benchmark.gd) |
| 🔴 | ⚠️ **A prova quente 2+5 está ligada ao jogo, foi perfilada e continua a reprovar.** Antes de mexer, scripts eram a maior fatia: **8,222/14,243 ms (57,7%)**; `WeaponAttach._process` 2,707 ms, mapa/save 2,299 ms e IA+física 1,714 ms. Foram retirados apenas cálculo/reflexão e redraw invisíveis, com imagem intacta. A prova real 1080p/Mobile sem VSync passou composição, visibilidade, input e deslocamento de 30,007 m; subiu de **75,2 para 100,0 fps** e o p95 caiu de **20,885 para 16,614 ms**, mas deu **p99 24,936 ms/pior 138,566 ms**: Lei 4 ainda vermelha. Máquina do Mateus (i7-1255U/16 GiB), 36 Claude e uma sessão Godot alheia; não é o i5-1334U/8 GiB do Rico. A tentativa FIFO seguinte coincidiu com `--import`/testes alheios (p99 33,332 ms) e não é causal. **Falta fora desta árvore:** ligar o gate ao `VERIFICAR.bat` com `user://` temporário e limpeza, repetir sem concorrência, e só fechar depois de p99 ≤16,67 ms no Rico. | [`graphics.json`](game/data/graphics.json) · [`animation_benchmark.gd`](game/src/tools/animation_benchmark.gd) · Lei 4 |
...
| 🔴 | **`VERIFICAR.bat` dá verde ao processo quando imprime vermelho.** Em 02-08, acesso rápido e fogueira activaram `FALHOU=1` e o batch mostrou `ALGUMA VERIFICACAO FALHOU`, mas terminou com código **0** porque o ramo final só faz `echo`/`pause`. O dono deve executar `exit /b 1` no ramo vermelho e `exit /b 0` no verde; sem isso CI/agentes aceitam uma suite que o próprio log rejeitou. | execução integral de `game/VERIFICAR.bat` 02-08 · regra “uma prova tem de bloquear” |
...
| 🟠 | **O guarda de coerência termina vermelho apesar de validar 2792 contratos sem erro:** `MAPA.md` aponta para `design/ideas/2026-07-31_0006__2026-07-30-23-52-46.ideas.md` e `design/transcripts/2026-07-31_0006__2026-07-30-23-52-46.md`, ambos ausentes; regenerar o mapa ou restaurar as fontes, nunca editar `MAPA.md` à mão | encontrado na árvore `fogueira-e-almas` · `node tools/check-coerencia.mjs` |
...
| ✅ | ~~**A resolução do `ArenaVorgar` foi reaberta por um merge inválido.**~~ **RESOLVIDO 02-08** — a fonte corrente importa sem declarações duplicadas, o audit da arena passa **26/26**, o ensaio focado passa **119/119** e a prova de jogo completo abaixo executa sem `SCRIPT ERROR`. O auto-teste global corrente passa **9765/9765**, logo já não depende da cache parcial descrita no achado original. | `game/src/world/arena_vorgar.gd` · `game/src/enemies/boss_vorgar.gd` |
...
| ✅ | ~~**A luta completa de vida cheia de Vorgar continuava por medir.**~~ **RESOLVIDO 02-08** — `arena_vorgar.tscn` arranca `gameplay.tscn`, e portanto `Main`, mundo, `Player`, HUD e `BossVorgar` reais; mantém **1950/1950 PV** do chefe e **442/442 PV** do Guerreiro, sem injecção de dano, invulnerabilidade, teleporte ou alteração da física. O piloto carrega as acções reais de lock-on, movimento, ataque, parry, esquiva e frasco. Vorgar caiu em **25 golpes**, **137,15 s** e **0 mortes**; o jogador recebeu golpes reais e curou-se. SEPARAR e JUNTAR emitiram o sinal `Label3D` visível e completaram a sequência espacial antes da morte. O resultado final foi visto no renderer: painel **0/1950 PV · DERROTADO** e toast **“Vorgar caiu. A fatia 1 está zerada.”** | duas provas completas aceites · ensaio Vorgar **119/119** · audit arena **26/26**, 02-08-2026 |
| 🔴 | **O percurso completo desde o início ainda continua por provar; a luta de Vorgar já não é o bloqueio.** `percurso.tscn` anda por `Input`, matou **7** inimigos e chegou vivo apenas a **4/17** destinos antes de morrer num encontro anterior. A prova dedicada agora confirma vida cheia, combate, morte e SEPARAR/JUNTAR dentro do jogo real, mas não substitui a travessia honesta desde o menu. | `scenes/percurso.tscn`, exit 1 · prova dedicada de Vorgar verde, 02-08-2026 |
| 🔴 | **As provas jogáveis ainda não pertencem todas ao corredor obrigatório.** Esta árvore não pode editar `game/VERIFICAR.bat`. Além de `percurso.tscn`, `filme-de-combate.tscn` e `sessao-de-jogo.tscn`, o dono do batch deve acrescentar `scenes/arena_vorgar.tscn -- --scene=vorgar --vorgar-full-fight-proof`, com `APPDATA`/`WORLDRPGS_TEST_USER_ROOT` temporários e limpeza da pasta criada. A prova recusa arrancar se o `user://` não estiver isolado. | restrição de propriedade desta ronda · regra “um teste que ninguém corre não é um teste” |
...
| ✅ | ~~**O auto-teste principal estava vermelho no marcador da Toca.**~~ **RESOLVIDO NO ESTADO CORRENTE 02-08** — uma execução isolada de `scenes/selftest.tscn` passou **9765/9765**, exit 0, sem a falha anterior. | `game/scenes/selftest.tscn` · `game/src/world/spawn_population.gd` |
...
Medição feita no jogo real com os dados efectivamente carregados pelo runtime: Guerreiro nível 1 (442 PV, 20 DEF, espada longa), Vorgar a 1950 PV, IA e física activas. A prova joga por `Input`, observa o HUD renderizado e só aceita a morte depois de SEPARAR e JUNTAR acontecerem por inteiro.

| Medida | ELES | NÓS | DIFERENÇA |
|---|---:|---:|---|
| Golpes de Vorgar para matar o jogador sem frasco | 3–4 | 3 pesados · 4–5 leves | dentro do alvo nos pesados; até +1 nos leves |
| Golpes da prova aceite para matar o chefe | 20–40 | 25 | dentro do alvo; mistura observada de ripostes e ataques leves |
...
| Duração da luta completa | — | 137,15 s | 2 min 17,15 s medidos, incluindo as duas fases espaciais |
| Mortes antes de matar Vorgar | — | 0 na prova aceite · 19 na calibração do piloto | duas execuções finais aceites sem morte; as 19 mortes anteriores tornaram explícitos os erros de esquiva, cura e leitura das fases |

**Decisão `[CODEX]`:** não alterar `enemies.json`, `combat.json` nem `attributes.json` nesta ronda. Razão: a luta observada já cai dentro dos 20–40 golpes da referência. O defeito real era de fio: quando o temporizador anti-kite ficava pronto sem existir um ataque `anti_kite_only` no catálogo, `BossVorgar` saltava sempre SEPARAR/JUNTAR. A correcção consulta o catálogo e deixa a máquina de fases avançar. Alternativa descartada: reduzir PV ou dano antes de provar a luta, porque esconderia o defeito sem tornar as fases jogáveis.

As quatro perguntas do fio solto:

1. **Como usa:** lock-on, movimento, ataque, parry, esquiva e frasco pelo mapa remapeável; são exactamente as acções enviadas pela prova.
2. **Como prova:** `arena_vorgar.tscn` arranca o jogo real, luta de vida cheia, exige SEPARAR/JUNTAR completos e verifica o resultado renderizado no HUD e no toast. Falta apenas o dono ligar o comando ao batch obrigatório.
3. **Arte e som:** Vorgar reutiliza o modelo/material do orc, `Label3D`/volumes da arena e os cues sintetizados já carregados; esta correcção acrescenta zero assets.
4. **Custo no Rico:** o baseline existente em Iris Xe, Mobile/Vulkan, 1920×1080, mediu **135,7 fps médios, p99 12,688 ms, pior 15,31 ms, zero frames >16,67 ms** em 15 s. A nova prova visual manteve 60 fps fixos e reportou **154 draw calls / 191 MB VRAM** enquanto executava o controlador real; como usa passo fixo, esse segundo valor prova estabilidade visual, não substitui o benchmark. A correcção de runtime acrescenta apenas uma passagem pelos ataques catalogados quando o anti-kite fica pronto; o restante piloto só existe sob a flag de prova.
...
| ✅ | ~~**As armas executáveis continuam invisíveis nas mãos e `presentation.model_asset` fica `null`.**~~ **CORPO JOGÁVEL RESOLVIDO 03-08** — `WeaponAttach` já prende o loadout ao Player e o preview herda o mesmo renderer; os 10 IDs executáveis declaram asset curado ou forma procedural e a cena real recusa qualquer falta. As variantes artísticas exactas de katana/Espadas continuam honestamente marcadas `missing_exact_*`: são produção final futura, não mão vazia hoje. | `game/data/weapons.json::presentation.model_asset` · `game/src/visual/weapon_visual.gd` · prova `repro-inicio.tscn` · Lei 4 |
...
| 🟠 | **O guarda global continua vermelho fora desta propriedade.** Mantém dois links partidos em `MAPA.md`; com a 7.ª origem, o cruzamento dos 19 JSON acrescenta as duas costuras esperadas `weapons.loadouts.evil_mage` e `economy.class_bias.profiles.evil_mage`, detalhadas abaixo. Há ainda um aviso: a `[TENSÃO]` de `spec/75` não aparece no `99`. Não editar `MAPA.md`, armas, economia, spec 75 ou 99 nesta árvore. | `node tools/check-coerencia.mjs` em 01-08 |
...
| 🟠 | **Invocações ainda não fecharam o gate 1080p/60 na Iris Xe.** Um `orc_spearman` real já conserva o próprio `MonsterVisual` ao morrer/levantar e a cena isolada prova input → morte → mesmo nó visual → aliado a andar. A amostra A/B de 1,5 s na Iris Xe, com outros agentes a disputar a máquina, deu corpo coerente **51,3 fps / média 19,506 ms / p99 138,966 ms / 173 draws** e skin antiga **52,8 fps / 18,937 ms / p99 128,192 ms / 173 draws**: não aumentou o máximo de draws, mas ambos falham e a amostra curta/contaminada não fecha Lei 4. ⚠️ A prova isolada também oscilou numa repetição final: falhou o limiar de movimento `>0,25 m` e a repetição imediata passou com **0,27 m**; o dono do repro deve colocar alvo/corpo num ponto sem obstáculo e publicar o deslocamento também na falha. O `necromancy_benchmark.gd` contorna cadáveres e instancia `RaisedEnemy` directamente, por isso não mede esta transferência. O dono do benchmark/prova deve manter 3/5/8 cadáveres reais levantados durante uma amostra quente de 60 s nos três presets; rede continua sem decisão de autoridade. | Lei 4 · [`52`](spec/52-mago-do-mal.md) §§9–10 · `repro-inicio.tscn -- --regra-de-ouro-isolada` 02-08 |
| 🔴 | **Falta a asserção visual directa na cena obrigatória porque a cena e o batch estão fora da propriedade desta árvore.** A produção compara a identidade do nó e só publica o levantado como aliado quando recebeu o próprio `MonsterVisual`; por isso o repro real fica vermelho se a transferência regredir. Também foi inspeccionado no jogo um orc lanceiro morto, escuro e dessaturado com a mesma arma/modelo. Ainda assim, `repro-inicio.tscn` não compara directamente modelo, escala e arma antes/depois, como a prova pedida exige. O dono da cena deve guardar essas três assinaturas antes de matar, premir `raise_dead` e compará-las no levantado; `VERIFICAR.bat` já chama a cena, mas precisa ainda do código de saída não-zero descrito acima. | fronteira de propriedade · `game/scenes/repro-inicio.tscn` · `game/VERIFICAR.bat` · regra “prova-o dentro do jogo” |
...
| 🟠 | **O controlador já liga o ecrã de nível ao descanso, mas a cena não liga o controlador:** `Bonfire` cria `LevelUpScreen`, chama `open_for_current()`, mantém a sequência sentada, não pausa o parceiro e devolve controlo ao fechar. `main.gd` continua a executar `_rest_at()` em paralelo e nunca instancia `Bonfire`, portanto o jogador entregue não alcança o ecrã. ⚠️ No `VERIFICAR.bat` de 02-08, o repro explícito ficou **10 verdes/1 vermelho**: exigiu o estado sentado final mas observou `Sitting_Enter`; o dono deve esperar a transição catalogada ou corrigir a passagem para `Sitting_Idle`, sem aceitar apenas o estado interno. Não criar tecla global: a acção continua `interact` no descanso. | `game/src/world/bonfire.gd` · `game/src/progression/bonfire_gameplay_repro.tscn` · lacuna 🔴 da cena real acima · [`20`](spec/20-interface.md) |
...
| 🟠 | **As fronteiras antigas falham em saves válidos com slot de arma `null`:** `InventorySystem.load_profile()`/`is_equipped()`, `Player.apply_inventory_state()` e `InventoryMenu` chamam `String(null)`. O Mago do Mal reproduz erros ao nascer e ao abrir a mochila. No `VERIFICAR.bat` de 02-08, `InventoryMenu._show_detail:218` voltou a abortar com `Invalid call 'String' constructor: adaga`; a prova partilhada acabou depois em “não forneceu boss e barra visível”, antes de chegar à necromancia. `EquipmentScreen`/`QuickSlots` usam apenas uma cópia legível com `""` e conservam o `null` canónico no save; os donos das três fronteiras comuns devem aceitar `null` como mão vazia e repetir o corredor completo. | `game/src/autoload/inventory_system.gd:265,391` · `game/src/player/player.gd:896` · `game/src/ui/inventory_menu.gd:182-258` · saída de `repro-inicio.tscn` 02-08 |
...
### Armas visíveis no equipamento — protocolo do [`31`](spec/31-referencias.md)

| | Eles | Nós antes desta árvore | Diferença / nossa versão |
|---|---|---|---|
| **Catalisadores legíveis** | DS2 agrupa sinos como ferramentas de feitiço e DS3 separa talismãs e sinos em categorias próprias; a distinção operacional existe antes dos números ([Chimes — DS2 Wiki](https://darksouls2.wiki.gg/wiki/Chimes), [Spell Tools — DS3 Wikidot](https://darksouls3.wikidot.com/catalysts)) | O jogador já recebia sino/talismã, mas o preview tinha uma lista manual de cinco IDs. `talisma` emitia aviso e deixava a mão vazia no ecrã de equipamento | Sino = boca cónica de bronze + badalo; talismã = placa irregular de ferro + cordão; escudo = disco largo importado. A auditoria compara a geometria renderizável e recusa que talismã, sino ou escudo tenham a mesma assinatura de silhueta |
| **Autoridade do conteúdo** | As ferramentas aparecem no mesmo catálogo operacional das restantes armas, sem depender de uma lista paralela de render | `WeaponVisual.WEAPON_SCENES` repetia cinco IDs; o runtime tinha **10** executáveis (`slot` + `hands`) e a metade nova era invisível no preview | `[CODEX]` Cada executável declara `presentation.model_asset`; o renderer descobre todos no JSON e aceita `res://…` curado ou `procedural:<forma>`. Razão: a próxima arma sem modelo torna a prova vermelha automaticamente. Alternativa descartada: acrescentar sino/talismã ao dicionário manual, porque repetiria exactamente a origem da regressão |
| **Corpo visual** | A família comunica função pela forma antes de o jogador ler estatísticas | Não havia declaração para nenhum dos 10 executáveis; cinco funcionavam apenas porque o código conhecia o ID | Seis entradas ligam quatro modelos CC0 já curados (`adaga`, `espada_recta`, `pesada_corte`, `escudo_medio`); cajado, sino, talismã e lâmina curva usam as formas low-poly partilhadas com `WeaponAttach`. Não entrou pack nem binário novo |

**As quatro perguntas do fio solto:**

1. **Como usa?** O jogador abre a mochila, carrega em **EDITAR ACESSO RÁPIDO** e vê o boneco real rodar com as mãos escolhidas; Feiticeiro e Mago do Mal já começam com sino/talismã. Não nasceu tecla nova.
2. **Como se prova?** `VERIFICAR.bat` já chama `repro-inicio.tscn`; essa cena arranca a criação e o mundo reais, o jogador abre a mochila pelo botão real e o `SubViewport` executa a auditoria dos 10 IDs. Mutação controlada de `talisma.model_asset = null` devolveu **exit 1**, nomeou `talisma` e limpou os nove slots de teste; reposto o campo, a cena imprimiu **10 armas executáveis visíveis**. A sonda isolada passou **11/11** e o auto-teste global **9765/9765**, mas a execução completa do repro continua vermelha depois desta prova por duas falhas alheias registadas abaixo.
3. **Arte e som?** Quatro GLTF KayKit CC0 já importados e quatro formas geométricas do renderer existente; zero asset comercial, zero caminho absoluto e zero som novo. O áudio de ataque/conjuração continua nas cues existentes.
4. **Quanto custa no Rico?** Iris Xe, Vulkan Mobile, 1920×1080, VSync off, 2 s de aquecimento + 5 s úteis: corpo base **269,8 fps / 3,706 ms / p99 6,522 ms / 8 draws**; espada + talismã **239,5 fps / 4,175 ms / p99 7,246 ms / 12 draws**. Diferença observada: **+0,469 ms, +4 draws, +414 primitivas e +4,0 MiB VRAM**. A auditoria dos 10 modelos custou **22,373 ms** e, por exceder um frame, corre apenas no `repro-inicio`/sonda isolada — nunca ao abrir o menu normal.

| Estado | Lacuna encontrada nesta prova | Dono / saída |
|---|---|---|
| 🔴 | Depois de a auditoria visual passar, `repro-inicio.tscn` ainda termina com `Invalid call 'String' constructor` em `InventoryMenu._show_detail()` ao descrever `adaga`, e a prova paralela deixa de encontrar o boss/barra. Isto impede `VERIFICAR.bat` inteiro de ficar verde nesta worktree, embora o passo das armas já tenha corrido e a mutação prove o seu código de saída | donos de `game/src/ui/inventory_menu.gd` e `game/src/ui/quick_slots_gameplay_proof.gd`; reproduzir com `VERIFICAR.bat --rapido --sem-pausa` e não mascarar o primeiro erro |
| 🟠 | `weapon_attach_benchmark.gd` deriva `is_two_handed` apenas de `staff.hands == 2`; por isso a primeira fotografia com cajado escondeu sino/talismã apesar de os loadouts reais ocuparem a secundária. A captura válida teve de usar uma espada de uma mão | dono de `game/src/visual/weapon_attach_benchmark.gd`; derivar a empunhadura do mesmo estado real usado pelo Player e provar staff+sino/staff+talismã |
| 🔴 | A etapa 5 do agregador também fica em **10/11** isoladamente: vida, frascos, inimigo, save e ecrã de nível passam, mas a prova espera `Sitting` e observa `Sitting_Enter` | dono de `game/src/progression/bonfire_gameplay_repro.*`/animação; decidir se deve aguardar a transição ou se o estado visível esperado mudou, sem enfraquecer a asserção |
...
2. **Como se prova:** `src/vfx/spell_cast_gameplay_film.tscn` monta `scenes/gameplay.tscn`, o `Player`, o mundo e inimigos reais; arranca como Mago do Mal, anda, carrega na acção `cast` e grava **52 frames físicos consecutivos**. Exige os quatro clips renderizados, compara cada imagem com o mundo antes da conjuração e passou com **440 pixels vermelhos novos no instrumento e 207 no corpo antes de existir projéctil, mais 287 no voo** (mínimos 100/50/40); a inspecção viu ainda o impacto baixar o orc de **135 para 114 PV**. O agregado oficial passou **9765/9765**. A prova paralela `src/spells/spell_game_integration.tscn` continua 12/13 pela expectativa alheia de mão secundária registada abaixo.
...
4. **Quanto custa na máquina do Rico:** Iris Xe, Mobile/Vulkan, 1920×1080, sem VSync, 6 s de aquecimento + 12 s medidos, Dardo + Ruína + Agulha e 39 clarões visíveis: **294,7 fps médios**, **3,394 ms médios**, p95 **6,220 ms**, p99 **8,599 ms**, pior frame **12,947 ms**, **26 draws**, 198 objectos e +3 213 871 bytes de memória estática. Só os três feitiços medidos residiam: 2 malhas + 2 materiais + 4 recursos. O primeiro ensaio, com dois draws redundantes e mais processos concorrentes, falhou p99 a 17,866 ms; foi preservado no relatório e a diferença não é atribuída só ao VFX. Ver [`medição`](game/src/vfx/benchmark_cast_iris_xe_2026-08-02.json).
...
|---|---|---|
| 🔴 | ⭐ **As provas jogáveis de magia ainda não pertencem ao corredor obrigatório.** `game/src/spells/spell_game_integration.tscn` prova entrega/impacto e `game/src/vfx/spell_cast_gameplay_film.tscn` prova as quatro fases e lê os **pixels renderizados** no foco, no corpo e entre instrumento/alvo; esta árvore não possui `game/VERIFICAR.bat`. Pela regra nova, a entrega só fica gate-completa quando o dono as ligar. | Acrescentar ambas antes da guarda final. ⚠️ O filme VFX deve correr com renderer real, não `--headless`: `"%GODOT%" --display-driver windows --audio-driver Dummy --path . --rendering-method mobile src/vfx/spell_cast_gameplay_film.tscn || set FALHOU=1`. Headless sai vermelho de propósito porque não emite `frame_post_draw`. A passagem final gravou 52/52 frames, validou pixels 440/207 antes do disparo e 287 no voo, usou apenas slots 13000–13999, validou `profile_id` antes de apagar e deixou zero ficheiros desses slots. |
...
| ⚠️ | **[TENSÃO] “toda magia vermelha paga PV” não tem número executável no catálogo.** O filme novo mostra fios vermelhos a saírem do torso para o foco, mas a própria prova observou `420/420 PV` antes e depois de lançar Agulha: é sinal corporal sem pagamento mecânico, portanto **o preço em PV não está provado**. `Lança Negra` declara 38 mana e nenhum custo de vida; o orçamento decidido de `spells.json::_rules.dark_mage_health_budget` é partilhado apenas por levantar morto, levantar chefe e Voto de Sangue. Esta árvore não inventou uma percentagem de PV para o projéctil. | `[CODEX]` Conservar o contrato actual até decisão: projécteis vermelhos pagam mana; necromancia/Voto pagam PV pelos campos já declarados. Alternativa, se Mateus quis PV em **cada** feitiço vermelho: o dono de `spells.json` deve declarar `health_cost_fraction` por ficha e o runtime deve mostrar/recusar o custo antes do compromisso. |
...
| 🔴 | **O filme vê o corpo levantar a mão esquerda vazia enquanto o cajado atravessa as costas; agora dois/três fios vermelhos finos saem do torso e apertam-se no foco, o que torna a extracção legível, mas também evidencia que instrumento e gesto não formam uma silhueta única.** `spell_cast_vfx.gd` usa correctamente `main_weapon_tip_position()`; mover/prender o instrumento durante a pose pertence ao dono de `weapon_attach.gd`/`character_visual.gd`, que esta tarefa foi proibida de editar. | Durante `casting_enter/idle/shoot/exit`, prender o catalisador à mão que o clip realmente anima ou escolher um clip compatível com a mão do instrumento; voltar a filmar de ombro e exigir que mão, foco e origem do projéctil coincidam no mesmo ponto. |
| 🟠 | **O save real de `evil_mage` entrou no filme só com os três favoritos azuis; Agulha teve de ser equipada temporariamente a partir de `classes.evil_mage.starting_spells`.** O teste não escreveu uma lista manual e a tecla real conjurou depois disso, mas o kit vermelho inicial continua a perder-se no percurso save → inventário → Player. | O dono do kit deve fazer `starting_spells` chegar aos favoritos sem a cena de prova os completar; depois a prova remove esse remendo temporário e continua a descobrir a magia pelo catálogo. |
| 🟠 | **`spell_game_integration.tscn` fecha agora 12/13 porque a asserção ainda exige cajado sem secundária, mas o kit real já aparece como `Cajado + talismã`.** Os outros 12 checks — cena, input, foco, rasto, relógio, dano e impacto azul/vermelho — passam. | O dono de `game/src/spells/` deve comparar a mão secundária com o instrumento inicial lido do catálogo, não com vazio. |
...
| 🟠 | O modo completo de `repro-inicio.tscn` continua bloqueado pela prova paralela de acesso rápido: `InventoryMenu._show_detail()` lança `Invalid call 'String' constructor: adaga` e, depois da virtualização, a prova procura `gameplay.boss`/barra antes de o guardião estar materializado. Não alterado porque pertence a outros ficheiros/agentes. **Reproduzido em 03-08 no passo 4/12: faz o `VERIFICAR.bat --rapido` devolver 1, embora o passo 5 da fogueira termine 11/11.** | `game/src/ui/inventory_menu.gd:218` · `game/src/ui/quick_slots_gameplay_proof.gd:299-313`; a passagem isolada da Regra de Ouro acima fica verde. |
| ✅ | ~~O modo completo de `repro-inicio.tscn` estava bloqueado pela prova paralela de acesso rápido.~~ A mochila trata `familia` como texto e `maos`/`alcance_m` como números vindos do JSON, abre e fecha pela ação real, mostra a adaga no detalhe, troca o atalho/equipamento, usa o frasco e ataca pela tecla até reduzir e retirar a barra. | `game/src/ui/inventory_menu.gd` · `game/src/ui/quick_slots_gameplay_proof.gd` · `scenes/repro-inicio.tscn` → `ARRANQUE + NECROMANCIA OK`, nove artefactos de save da prova apagados. |
| 🔴 | **A fábrica de produção do guardião continua fora desta árvore:** `SpawnPopulation._spawn_placement()` chama `Main._spawn()` sem actor e `Main._spawn()` usa `Enemy.new()`. A prova materializa cedo um único `BossVorgar`, descoberto no plano/catálogo, porque os dois fluxos paralelos precisam do mesmo alvo; isto não prova que andar até à arena instancia a subclasse correcta. | O dono de `game/src/main.gd`/`game/src/world/spawn_population.gd` deve escolher a subclasse a partir da ficha `is_boss`, sem lista manual de IDs, e provar no percurso real que chegar à arena cria `BossVorgar` e mostra a barra. |

## Minimapa legível — CODEX, 02-08-2026

| Estado | Resultado / lacuna | Prova exacta |
|---|---|---|
| ✅ | O HUD real deixou de desenhar uma mancha âmbar: o caminho descoberto tem linha e contorno contínuos, a fogueira é chama, a Toca é portão, a arena é coroa e a barra de 10 m dá escala. O nevoeiro continua preto e nenhum marco aparece antes de ser descoberto. | `game/src/ui/map_surface.gd` · `game/src/ui/navigation_hud.gd` · `game/data/world.json::map_reading.runtime.cartography` · capturas locais regeneráveis `game/captures/minimapa-30s.png` e `07-mapa-grande.png`. |
...
| 🟠 | A parede de **5,06 m** foi identificada sem mover a rota: o chão exterior de Brumal era um único `BoxShape3D` de 220×220 m a `y=0`, por cima da rampa da Toca. O jogador ficava em `y≈0` em vez de descer para `y=-3,2` e batia na parede norte de `LairCollision`. `Greybox` passou a abrir no chão visual e físico uma passagem derivada de `Lair.MAIN_ROUTE`/`MODULE`; no jogo real o mesmo ponto ficou em `y=-3,205` e o destino 09 foi alcançado. **Ainda não há 17/17**, logo a tarefa não é declarada fechada. | Não se alterou waypoint nem altura de degrau. A prova canónica continua a ser `scenes/percurso.tscn`; só pode ficar verde quando imprimir `destino 17/17 alcancado a pe`. |
| 🔴 | [CODEX] O passo 9 é intermitentemente interrompido antes/depois da entrada por população fora da geometria: um corpo vivo pode ser retirado sem `died` durante o confronto. A instrumentação apanhou um brutamontes desmaterializado a **7,85 m** do jogador porque o seletor preferiu cinco colocações mais próximas; ao conservar experimentalmente todos os corpos ativos, o piloto ficou cercado e morreu nos destinos 05/08. Essa alteração foi revertida por tocar no equilíbrio de população/combate fora desta árvore. Na execução final, já sem instrumentação, morreram seis inimigos por sinal e o piloto chegou a **8/17**, mas morreu contra o lanceiro da primeira sala. | O dono da população deve decidir uma política de estabilidade de confronto que respeite simultaneamente o teto catalogado e não forme uma multidão persistente. Proposta [CODEX]: reservar primeiro o alvo/atores já em confronto e só preencher lugares livres com novas colocações; alternativa: fazer o ator regressar visivelmente ao `home` antes de o virtualizar. Repetir depois o passo 9 inteiro com `user://` isolado. |
| 🟠 | A alteração do chão foi medida na Iris Xe, Mobile/Vulkan, 1920×1080, médio, VSync off, 8 s de aquecimento + 20 s: **114,5 fps**, p95 **12,196 ms**, p99 **14,038 ms**, 0,4% acima de 16,67 ms, 139 draws e 149,9 MiB VRAM. O p99 passa a Lei 4 nesta amostra, mas havia **10 processos Godot** de agentes em paralelo, por isso não é um A/B causal limpo. | O chão continua em dois lotes `MultiMesh` (base + relva), agora com quatro painéis, e quatro caixas de colisão. Repetir sozinho na Iris Xe antes de atribuir diferença temporal à passagem. |
...
|---|---|---|
| 🟠 | O modo por defeito de `weapon_attach_benchmark.gd` não normaliza `offhand: null`: `String(loadout.get("offhand", ""))` lança erro, mas o processo continua, imprime `actors:9` e termina com código 0 apesar de ter interrompido a construção. Esses resultados podem parecer válidos sem o serem. O dono deve converter `null` em vazio e fazer a medição falhar perante qualquer erro de construção. | `game/src/visual/weapon_attach_benchmark.gd:109`; reprodução Mobile/Vulkan na Iris Xe com `--mode=base` e `--mode=armed`, 02-08-2026. A prova válida desta tarefa usou explicitamente `--weapon=longsword --offhand=shield`: um actor nos dois lados, sem `SCRIPT ERROR`. |
| ✅ | O HUD real deixou de desenhar uma mancha âmbar: o caminho descoberto tem linha e contorno contínuos, a fogueira é chama, a Toca é portão, a arena é coroa e a barra de 10 m dá escala. O nevoeiro continua preto e nenhum marco aparece antes de ser descoberto. | `game/src/ui/map_surface.gd` · `game/src/ui/navigation_hud.gd` · `game/data/world.json::map_reading.runtime.cartography` · capturas locais regeneráveis `game/captures/minimapa-30s.png`, `minimapa-toca.png` e `minimapa-arena.png`. |
| ✅ | Uma única prova arranca o jogo real em duas fases catalogadas. Em `perf`, o jogador carrega em `move_forward` + `dodge_sprint`, fotografa aos 30,00 s/175,5 m e chega à Toca aos 36,55 s/208,2 m. Depois o próprio jogo reinicia em `vorgar`, anda 7,2 m por input e exige a coroa. O resultado final contou 1 075 pixels de caminho, 655 da chama, 124 do portão, 83 da coroa e 139/127 da escala; as três capturas foram abertas e inspeccionadas. | `--bench --scene=perf --warmup=0 --seconds=95 --map-proof` → `[MAP_PROOF] PHASE PASS perf -> vorgar` + `[MAP_PROOF] PASS`; `user://` temporário auditado e removido. |
| 🟠 | A prova jogável ainda não está no comando canónico porque `game/VERIFICAR.bat` e as cenas de teste não pertencem a esta árvore. O runner vive no próprio `NavigationHud` e já falha com código 1 se a captura não mostrar o resultado, mas outro dono tem de acrescentar o comando acima ao batch **dentro de `APPDATA`/`WORLDRPGS_TEST_USER_ROOT` temporários, com limpeza mesmo se falhar**. A linha Godot isolada não é segura: o arranque normal cria `slot_00`. | `game/VERIFICAR.bat` não contém `--map-proof`; não foi alterado por respeito à lista de ficheiros autorizada. Copiar o invólucro de isolamento usado pela prova da fogueira, não apenas o comando. |
